...
const envExample = path.join(SMS_ROOT, "backend", ".env.example");
const env = path.join(SMS_ROOT, "backend", ".env");
const envExampleDest = path.join(DEST, "backend", ".env.example");
if (fs.existsSync(envExample)) {
  fs.copyFileSync(envExample, envExampleDest);
  console.log("Synced backend .env.example");
} else if (fs.existsSync(env)) {
  fs.copyFileSync(env, envExampleDest);
  console.log("Created backend .env.example from backend .env");
}
